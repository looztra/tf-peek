...
- [ ] 1.1 In `src/tf_peek/main.py`, add a string `Enum` (e.g. `Action`) with members
...
- [ ] 1.2 Add `fail_on_critical: bool = typer.Option(False, "--fail-on-critical", help=...)` and
      `fail_on_critical_on: list[Action] = typer.Option([], "--fail-on-critical-on", help=...)` to
      `generate()`.
- [ ] 1.3 In the existing per-resource loop in `generate()` (around `main.py:380-429`), alongside
...
- [ ] 1.4 After the report is rendered and written/echoed (after the existing `if output_file: ...
...
- [ ] 1.5 Confirm (existing typer/click behavior, no new code) that an invalid
...
- [ ] 2.1 `test_fail_on_critical_absent_exits_zero` — plan with a critical delete, no flags passed,
      asserts `exit_code == 0` and report content unchanged from today's golden output.
- [ ] 2.2 `test_fail_on_critical_triggers_on_default_scope` — plan with a resource whose
...
- [ ] 2.3 `test_fail_on_critical_no_critical_resources_exits_zero` — `--fail-on-critical` passed, no
      `tier == "critical"` resources in the plan, asserts `exit_code == 0`.
- [ ] 2.4 `test_fail_on_critical_on_scoped_action_present` — `--fail-on-critical-on delete`, a
...
- [ ] 2.5 `test_fail_on_critical_on_divergence_from_rendered_section` — `--fail-on-critical-on
...
- [ ] 2.6 `test_fail_on_critical_on_multiple_actions` — two `--fail-on-critical-on` occurrences,
      asserts a resource matching either triggers `exit_code == 3`.
- [ ] 2.7 `test_fail_on_critical_on_invalid_action_is_usage_error` — `--fail-on-critical-on destroy`,
      asserts `exit_code == 2` and no report is written even if `--output` was also passed.
- [ ] 2.8 `test_fail_on_critical_on_takes_precedence_when_both_flags_passed` — both
      `--fail-on-critical` and `--fail-on-critical-on delete` passed, only a critical replace present,
      asserts `exit_code == 0`.

## 3. Documentation

- [ ] 3.1 `docs/reference/cli.md`: add `--fail-on-critical` and `--fail-on-critical-on ACTION` rows
      ...
      `--fail-on-critical-on`.
- [ ] 3.2 `docs/reference/cli.md`: add exit code `3` to the exit-codes table
      ("Critical gate triggered (`--fail-on-critical`/`--fail-on-critical-on`); the report was still
      generated").
- [ ] 3.3 `docs/reference/cli.md`: add a worked example, e.g.
      `tf-peek plan.json --fail-on-critical-on delete` in a CI step, with a one-line note on
      branching on exit code `3` vs `1`.

## 4. Verification

- [ ] 4.1 Run `poe lint:all`.
- [ ] 4.2 Run `poe test` (full suite, including the new gate tests).
- [ ] 4.3 Manually run `tf-peek --fail-on-critical-on delete` and `--fail-on-critical-on destroy`
      against a sample plan to confirm exit codes `3`/`0`/`2` match the spec scenarios.
- [ ] 4.4 Run `pre-commit run --all-files markdownlint-cli2` (docs lint clean).
